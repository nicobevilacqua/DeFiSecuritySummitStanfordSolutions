...
contract BorrowSystemInsecureOracle {
    ...
    constructor(
        ...
        address _tokenInsecureum,
        address _tokenBoring
    ) {
    ...
    function isSolvent(address user) public view returns (bool) {
        ...
        uint256 collateralValue = token0Deposited[user] +
            (token1Deposited[user] * _tokenPrice) /
            _base;

        uint256 maxBorrow = (collateralValue * 100) / 90; // 90% LTV
        uint256 borrowed = token0Borrowed[user] +
            (token1Borrowed[user] * _tokenPrice) /
            _base;
